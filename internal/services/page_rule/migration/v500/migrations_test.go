...
// TestMigratePageRule_V4ToV5_Basic tests basic field migrations from v4 to v5.
// This test validates:
...
func TestMigratePageRule_V4ToV5_Basic(t *testing.T) {
	t.Run("from_v4_latest", func(t *testing.T) {
		...
		target := "tf-test-" + rnd
		tmpDir := t.TempDir()
		testConfig := fmt.Sprintf(v4BasicConfig, rnd, zoneID, target)
...
// TestMigratePageRule_V4ToV5_CacheKeyFields tests 5-level deep nested structure transformation from v4 to v5.
// This test validates:
...
func TestMigratePageRule_V4ToV5_CacheKeyFields(t *testing.T) {
	t.Run("from_v4_latest", func(t *testing.T) {
		...
		target := "tf-test-" + rnd
		tmpDir := t.TempDir()
		testConfig := fmt.Sprintf(v4CacheKeyFieldsConfig, rnd, zoneID, target)
		version := os.Getenv("LAST_V4_VERSION")
		sourceVer, targetVer := acctest.InferMigrationVersions(version)
