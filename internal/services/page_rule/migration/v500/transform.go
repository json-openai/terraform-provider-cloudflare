...
func Transform(ctx context.Context, source SourceV4PageRuleModel) (*TargetV5PageRuleModel, diag.Diagnostics) {
...
func transformActions(ctx context.Context, source SourceV4ActionsModel) (*TargetV5ActionsModel, diag.Diagnostics) {
	var diags diag.Diagnostics

	target := &TargetV5ActionsModel{}
	...
	if len(source.ForwardingURL) > 0 {
		target.ForwardingURL = &TargetV5ForwardingURLModel{
...
func transformCacheKeyFields(ctx context.Context, source SourceV4CacheKeyFieldsModel) (*TargetV5CacheKeyFieldsModel, diag.Diagnostics) {
	var diags diag.Diagnostics

	target := &TargetV5CacheKeyFieldsModel{}

	// Level 2: cookie (TypeList MaxItems:1 → pointer)
	if len(source.Cookie) > 0 {
		cookieTarget := &TargetV5CacheKeyFieldsCookieModel{}
	...
	if len(source.Header) > 0 {
		headerTarget := &TargetV5CacheKeyFieldsHeaderModel{}
	...
	if len(source.Host) > 0 {
		hostTarget := &TargetV5CacheKeyFieldsHostModel{
	...
	if len(source.QueryString) > 0 {
		qsTarget := &TargetV5CacheKeyFieldsQueryStringModel{}
	...
	if len(source.User) > 0 {
		userTarget := &TargetV5CacheKeyFieldsUserModel{
...
// v5: {"200": "3600", "404": "300"}
func transformCacheTTLByStatus(ctx context.Context, source []SourceV4CacheTTLByStatusModel) (types.Map, diag.Diagnostics) {
	var diags diag.Diagnostics
